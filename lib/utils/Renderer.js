...
var renderTemplateSafe = function renderTemplateSafe(element, options, generateCustomTemplate) {
...
var renderTemplateUnsafe = function renderTemplateUnsafe(options, generateCustomTemplate) {
  console.warn('Accepting bodyContent as an option is deprecated and will be removed ' + 'in the next minor release. Instead, pass the top-level ReactElement ' + 'as the first parameter, i.e. Oy.renderTemplate(<Template />, options)');
  return generateCustomTemplate ? generateCustomTemplate(options) : _HTML2.default.generateDefaultTemplate(options);
};

exports.default = {
  renderTemplate: function renderTemplate() {
    var rawHTML = _react2.default.isValidElement(arguments.length <= 0 ? undefined : arguments[0]) ? renderTemplateSafe.apply(undefined, arguments) : renderTemplateUnsafe.apply(undefined, arguments);
